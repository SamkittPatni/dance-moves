import VideoLink from "@/app/ui/steps/videolink";
import FigureList from "@/app/ui/steps/figurelist";
import { Suspense } from "react";

export default function Page() {
  const precedingFigures = [
    { name: "Promenade and Counter Promenade Runs", href: "promenade_and_counter_promenade_runs" },
    { name: "Foot Changes", href: "foot_changes" },
  ];

  const followingFigures = [
    { name: "Samba Locks", href: "samba_locks" },
    { name: "Cruzados Walks and Locks", href: "cruzados_walks_and_locks" },
    { name: "Contra Bota Fogos", href: "contra_botafogos" },
    { name: "Shadow Travelling Voltas", href: "shadow_travelling_volta" },
    { name: "Shadow Circular Voltas", href: "shadow_circular_volta" },
    { name: "Foot Changes", href: "foot_changes" },
  ];

  return (
    <>
      <div className="flex flex-col w-full h-full bg-black/75 text-justify md:px-10">
        <h1 className="font-serif font-bold text-center">Drop Volta</h1>
        <hr className="mx-3 pb-5 border-gray-600" />
        <div className="px-3">
          {/* Closed Changes (also simply called a waltz basic) can start from both LF and RF.         */}
        </div>
        <div className="flex flex-col justify-center py-5">
          <h2 className="font-bold text-2xl text-center pb-2">Video</h2>
          <hr className="mx-3 pb-5 border-gray-600" />
          <Suspense fallback={<p>Loading video...</p>}>
            <VideoLink videoID="RFCVNgJUxfI" start={0} end={-1} />
          </Suspense>
        </div>
        <div className="flex md:flex-row flex-col py-5">
          <div className="w-full text-center md:mx-2 py-5 md:bg-black/75">
            <h2 className="font-bold text-2xl pb-2">Preceding Figures</h2>
            <hr className="mx-3 pb-2 border-gray-600" />
            <FigureList steps={precedingFigures} />
          </div>
          <div className="w-full text-center md:mx-2 py-5 md:bg-black/75">
            <h2 className="font-bold text-2xl pb-2">Following Figures</h2>
            <hr className="mx-3 pb-2 border-gray-600" />
            <FigureList steps={followingFigures} />
          </div>
        </div>
      </div>
    </>
  );
}

// Preceding Figures
// Promenade and Counter Promenade Runs
// Foot Changes

// Following Figures
// Samba Locks
// Cruzados Walks and Locks
// Contra Bota Fogos
// Shadow Travelling Voltas
// Shadow Circular Voltas
// Foot Changes